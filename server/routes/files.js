...
// Keys look like "<folder>/<uuid>.<ext>". Reject traversal, artifacts, anything else.
const KEY_RE = /^[a-z0-9][a-z0-9/_-]*\.[a-z0-9]{1,8}$/i;

router.get('/*', async (req, res) => {
  const key = req.params[0] || '';
  if (!KEY_RE.test(key) || key.includes('..') || key.startsWith('artifacts/')) {
